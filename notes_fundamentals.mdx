...
### 1.2 OpenAI API and playground parameters
...
### 2.5 Add chat history (context)

- you can build this by simply pushing the user and assistant messages to an array and then using that array as the context for the next message
-The removeOlderTokens function is designed to manage the size of the context array by ensuring that the total number of tokens does not exceed a specified maximum (MAX_TOKENS). Here's a step-by-step explanation of how it works and its purpose:
Purpose
The function ensures that the context used for generating chat completions does not become too large. This is important because:
- 1. API Limits: The OpenAI API has limits on the number of tokens that can be processed in a single request.
- 2.Performance: Keeping the context within a manageable size can improve performance and response times.
- 3. Cost: Reducing the number of tokens can help manage costs, as API usage is often billed based on the number of tokens processed.
